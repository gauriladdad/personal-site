...
5.  Click on your new bucket > **Permissions** tab.
6.  Scroll to **Cross-origin resource sharing (CORS)** and click **Edit**. Paste this JSON:
    ```json
    [
      {
        "AllowedHeaders": ["*"],
    ...
    ```
    _(Ideally, replace `_`in AllowedOrigins with your actual domain`https://news.gattani.ca` for better security).\*
...
5.  Click the Role name to open IAM.
6.  Click **Add permissions** > **Attach policies**.
7.  Search for `AmazonS3FullAccess` (or create a stricter policy) and attach it.
...
1. Find the RSS feed URL (e.g., BBC: http://feeds.bbci.co.uk/news/[category]/rss.xml)
2. Add to CATEGORY_FEEDS:

```
"category_id": {
    "name": "Display Name",
    "url": "https://feed-url-here",
    "type": "rss"
}
```

3. Add to DEFAULT_MAX_STORIES:

```
"category_id": 5
```

Or use env var to override without redeploying:

```
{"top": 8, "tech": 5, "science": 5, "canada": 5, "sports": 5}
```
